...
fig, ax = plt.subplots(N, M, figsize=(12, 8), sharex=True, sharey=True)
...
cbar_ax = fig.add_axes([0.92, 0.55, 0.015, 0.35])  # [left, bottom, width, height]
...
cbar2.set_label('Absolute Error (m)', rotation=270, labelpad=15)
...
ax[0,0].set_ylabel('Sample Ratio', fontsize=AXIS_FONT_SIZE)
ax[1,0].set_ylabel('DepthPrompting', fontsize=AXIS_FONT_SIZE)
ax[2,0].set_ylabel('Abs. Error (DP)', fontsize=AXIS_FONT_SIZE)
ax[3,0].set_ylabel('DepthFormer + CS', fontsize=AXIS_FONT_SIZE)
ax[4,0].set_ylabel('Abs. Error (CS)', fontsize=AXIS_FONT_SIZE)
